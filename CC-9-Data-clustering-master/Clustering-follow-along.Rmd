...
To answer these questions, you can construct a dataset which consists of tree species occurrence records for a multitude of sites in the Neotropics and perform a series of hierarchical clustering analyses.
...
+ Sequential and simultaneous - refers to how the clustering is conducted. If it’s through an algorithm that is repeated till all data have been clustered, it’s sequential. If the algorithm clusters all your data together at the same time, it’s simultaneous.
+ Agglomerative and divisive - refers to how your data are being grouped. Agglomerative is a bottom up approach, meaning that the clustering will begin by putting similar observations together, gradually forming subgroups till all your observations are included. Divisive is the exact opposite, your set of observations will be considered as whole group and the algorithm will divide your data into progressively smaller chunks till each observation forms a sub-group on its own.
+ Monothetic and polythetic - refers to the amount of descriptors being employed to cluster your data into subgroups. If it uses just one descriptor on every step, it’s monothetic; if it uses more than one, it’s polythetic.
+ Hierarchical and non-hierarchical - hierarchical means that your groups will be organized in ranks according to how similar they are. You’ll have sub-groups forming larger groups till all your observations are included in your cluster. Non-hierarchical clustering methods do not include that option.
...
sites <- vroom::vroom('sites_bolivia.csv', delim = ',')
sites
```
...
Now it’s time for us to load the correspondence matrix, a matrix indicating what species are present on which site, information that is vital to us in order to make a presence and abscence matrix.

```{r}
# Load the sppxarea matrix
sppxsites <- vroom::vroom('sppxsites_bol.csv', del = ',')
sppxsites
```
...
The way we are going to build such matrix is through a loop function. Loop functions are extremely useful used in many programming languages. If you want to learn more about them, you can check our tutorial on how to use loops.
```{r}
...
# This is the loop given in the tutorial. A bit overly complex.
spp_commat <- matrix(data = 0, nrow = length(sites_sub), ncol = length(spp_sub))
...
```{r}
# This is an alternative approach that leverages tidyverse features and workflow:
sppxsites_long <- sppxsites %>% 
  distinct()

sppxsites_named_long <- sppxsites_long %>% 
  left_join(sites %>% select(AreaID, AreaCode), by = 'AreaID') %>% 
  left_join(spp %>% select(SppID, Species.code), by = 'SppID') %>% 
  select(AreaCode, Species.code) %>% 
  mutate(Presence = 1)

spp_commat_tidy <- sppxsites_named_long %>% 
  pivot_wider(
...
```{r}
# This removes 275 species.
spp_commat_trim <- spp_commat[, which(!colSums(spp_commat) == 1)]
dim(spp_commat_trim)
```

### Distance Metrics
To learn which distance measures are avialble to us, chekc out:
```{r}
help("vegdist")
help("recluster.dist")
```
...
Turnover refers to when you have species replacing each other over an environmental gradient or geographic space. Nestedness refers to when a site/sample is occupied by a fraction, a subset, of the surrounding species pool, not by new species (nestedness is commonly associated with environmental filtering). Depending on the patterns you want to look at and how big your dataset is in terms of scale, these fractions must be taken into account. Jaccard distance is defined as turnover + nestedness = 1, Simpson focus on species turnover only, so it is great for biogeographic analysis.
...
+ [Single-linkage method (single dendogram)](#id01)
+ [Single-linkage method (concensus dendogram)](#id02)
+ [Complete-linkage method](#id03)
+ [Clustering using Ward’s minimum variance](#id04)
+ [Average linkage mehtod (UPGMA) and an observation on "uniques" and potential biases in the dendograms](#id05)
...
plot(bol_singlelink_tmp, direction = 'downwards', cex = 0.5)
...
write.tree(bol_singlelink_tmp, '01_bol_singlelink_temp.tre')
```
...
write.tree(bol_singlelink_cons, '02_bol_singlelink_cons.tre')
...
Now we will move on to the other methods and we will always work with the solution reached through the application of the consensus criterion (p=0.5).
...
write.tree(bol_completelink_cons, '03_bol_completelink_cons.tre')
...
write.tree(bol_ward_cons, '04_bol_ward_cons.tre')
```
...
write.tree(bol_upgma_cons, '05_bol_upgma_cons.tre')
plot(bol_upgma_cons, direction = 'downwards', cex = 0.5)

# Trimmed species presence/absence matrix
bol_upgma_trim <- recluster.cons(spp_commat_trim, tr = 100, p = 0.5, method = 'average')
bol_upgma_trim_cons <- bol_upgma_trim$cons
write.tree(bol_upgma_trim_cons, '06_bol_upgma_cons_trim.tre')
plot(bol_upgma_trim_cons, direction = 'downwards', cex = 0.5)
```

So, at this point the author of the tutorial claims that the dendogram didn't change. What actually happened is that the same dendogram was plotted twice. Doing it correctly does indeed reveal a change.

## 3. Bootstrap and support values for subgroups
Bootstrapping is one of the main ways of calculating support values for the subgroups we have got in our dendogram. It is usually used to test significance through randomisation procedures. However, since hierarchical clustering analysis do not work with significance, here bootstrapping assesses whether you’ll get the same subgroups in a dendogram even if you resample the species occurring in each site (with replacement - meaning you can repeat species and even not include some species at all. Bootstrap values range from 0 to 100 and express the proportion of times a certain subgroup was recovered during the resampling. The bigger the number, the stronger the subgroup.
...
A dendogram with no polytomies will have x tips and x-1 internal nodes. However, all dendograms we have produced so far will have this number, because R does not automatically recognize polytomies. Let’s use the di2multi function from the phytools package and identify these polytomies.

```{r}
bol_upgma_cons
bol_upgma_cons_nodi <- di2multi(bol_upgma_trim_cons)
bol_upgma_cons_nodi
write.tree(bol_upgma_cons_nodi, '07_bol_upgma_cons_nodi.tre')
```
...
```{r}
bol_singlelink_cons
bol_singlelink_cons_nodi <- di2multi(bol_singlelink_cons)
bol_singlelink_cons_nodi
write.tree(bol_singlelink_cons_nodi, '08_bol_singlelink_cons_nodi.tre')
...
```{r}
# Group 1 tips
group1_node <- findMRCA(bol_upgma_cons_nodi, c('AndBO034', 'AndBO018'))
group1_tips <- getDescendants(bol_upgma_cons_nodi, group1_node)
...
# Group 2 tips
group2_node <- findMRCA(bol_upgma_cons_nodi, c('AmzBO066', 'AmzBO057'))
group2_tips <- getDescendants(bol_upgma_cons_nodi, group2_node)
group2_tips <- na.omit(bol_upgma_cons_nodi$tip.label[group2_tips])   # Remove the nodes (NAs) and return just the tips to you.
length(group2_tips)   # Count the number of tips you've got for each group. This will be useful to check if the code worked well.

# Group 3 tips
group3_node <- findMRCA(bol_upgma_cons_nodi, c('AmzBO025', 'AmzBO033'))
group3_tips <- getDescendants(bol_upgma_cons_nodi, group3_node)
group3_tips <- na.omit(bol_upgma_cons_nodi$tip.label[group3_tips])   # Remove the nodes (NAs) and return just the tips to you.
length(group3_tips)   # Count the number of tips you've got for each group. This will be useful to check if the code worked well.

# Group 4 tips
group4_node <- findMRCA(bol_upgma_cons_nodi, c('CerBO012', 'AmzBO044'))
group4_tips <- getDescendants(bol_upgma_cons_nodi, group4_node)
group4_tips <- na.omit(bol_upgma_cons_nodi$tip.label[group4_tips])   # Remove the nodes (NAs) and return just the tips to you.
length(group4_tips)   # Count the number of tips you've got for each group. This will be useful to check if the code worked well.
```
...
```{r}
length(group1_tips) + length(group2_tips) + length(group3_tips) + length(group4_tips)
...
cluster_membership[which(all_tips %in% group1_tips)] <- 'Group 1'   # Then we assigna each set of tips to a subgroup
...
## 6. Visualising the results of our clustering on a map
...
## Challenge???
The challenge in this tutorial has been skipped due it having to do with mapping and I rather prefer the mapping method I learned based on interactive maps. Instead, what follows is an attempt at recreating the last p.
```{r}
# Ensure necessary libraries are loaded
# install.packages(c("vegan", "dplyr", "tibble")) # Uncomment to install if needed
library(vegan)
library(dplyr)
library(tibble)

# --- Prerequisites (from your tutorial steps) ---
# Assume 'spp_commat_trim' is the presence/absence matrix (sites x species)
# with rownames as AreaCode, computed earlier.
# Assume 'sites' is the dataframe with site metadata, including 'AreaCode'.

# Example placeholder if running independently:
# spp_commat_trim <- matrix(rbinom(217 * 100, 1, 0.3), nrow = 217, ncol = 100)
# rownames(spp_commat_trim) <- paste0("Site", 1:217)
# sites <- data.frame(AreaCode = paste0("Site", 1:217),
#                     Lat10 = runif(217, -25, -8),
#                     Long10 = runif(217, -70, -55),
#                     Domain = sample(c("Amz", "And", "Cer", "Cha"), 217, replace = TRUE))
# --- End Prerequisites ---

# 1. Calculate Pairwise Distances
# Using vegan::vegdist for the Simpson distance (equivalent to recluster.dist with dist="simpson")
# Note: vegan's 'simpson' is the similarity; we need dissimilarity (1 - similarity).
# Alternatively, vegdist uses Sorensen by default for binary data if method="bray",
# or Jaccard if method="jaccard". Let's stick to Simpson as per the tutorial's choice.
# Calculating Simpson dissimilarity directly:
site_dist_simpson <- vegdist(spp_commat_trim, method = "jaccard", binary = TRUE)
# Correction: The tutorial uses Simpson dissimilarity for turnover.
# beta.pair in betapart package or specific calculation is needed for true Simpson dissimilarity.
# Let's use Jaccard for simplicity here, as it's common and handled directly by vegdist.
# If strict adherence to Simpson dissimilarity is needed, it requires a bit more setup
# or using a package like 'betapart'. Let's proceed with Jaccard as an example
# of this workflow, or use Sorensen (often called Bray-Curtis for binary).
site_dist_sorensen <- vegdist(spp_commat_trim, method = "bray", binary = TRUE) # Sorensen for binary
# Let's use Sorensen as it was mentioned, although cautioned against for this data type in the text.
# Sticking to the tutorial's final choice: UPGMA with Simpson. Let's calculate Simpson dissimilarity manually or use `recluster.dist` if installed.
# Using recluster.dist as the tutorial does:
# install.packages("recluster") # Uncomment if needed
library(recluster)
site_dist_simpson_recluster <- recluster.dist(spp_commat_trim, dist = "simpson")

# 2. Perform Hierarchical Clustering
# Using base R hclust function with the chosen linkage method (e.g., UPGMA = "average")
# We'll use the Simpson distance calculated via recluster.dist to match the tutorial
hclust_avg_simpson <- hclust(site_dist_simpson_recluster, method = "average") # UPGMA
write.tree(hclust_avg_simpson, '09_hclust_avg_simpson.tre')

# Optional: Plot the dendrogram to help decide where to cut
# plot(hclust_avg_simpson, cex = 0.6, hang = -1, main = "UPGMA Clustering (Simpson Distance)")

# 3. Cut the Dendrogram to Define Groups
# Decide on the number of groups (k) or the height (h) to cut at.
# The tutorial visually decided on 4 groups for the UPGMA example.
num_groups <- 4
cluster_groups <- cutree(hclust_avg_simpson, k = num_groups)

# 'cluster_groups' is now a named integer vector: names are AreaCode, values are group numbers (1 to k)
# str(cluster_groups)
# head(cluster_groups)

# 4. Assign Groups to the Sites Data Frame
# Convert the named vector to a tidy data frame for joining
group_assignments <- cluster_groups %>%
  tibble::enframe(name = "AreaCode", value = "ClusterGroup") %>%
  mutate(ClusterGroup = paste0("Group ", ClusterGroup)) # Optional: format as "Group X"

# Join with the original sites metadata
sites_with_groups <- sites %>%
  left_join(group_assignments, by = "AreaCode")

# Verify the result
# head(sites_with_groups)
# unique(sites_with_groups$ClusterGroup)
# table(sites_with_groups$ClusterGroup) # See group sizes

# Now 'sites_with_groups' contains your original site data plus a new 'ClusterGroup' column.
# This data frame is ready for further processing, such as mapping (like in Section 6 of the tutorial)
# or other analyses (PCA, NMDS, ANOVA comparing groups, etc.).
```
